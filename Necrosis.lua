...
function Necrosis:SpellSetup()
  ...
  -- Number - number of ability tabs in the player's spellbook (e.g. 4 -- "General", "Arcane", "Fire", "Frost")
	for i = 1, GetNumSpellTabs(), 1 do
		...
		-- for each entry in the spell book between the offset (last spell of previos tab) and the offset + number of spells on this tab
		for j = offset+1, offset+numEntries, 1 do
			spellName, spellSubName, spellID = GetSpellBookItemName(j, BOOKTYPE_SPELL)
			...
			--
			-- For spells with numbered ranks, we compare ranks 1 to 1 for each spell.
			-- The higher rank is retained
			if spellSubName and not (spellSubName == " " or spellSubName == "") then
				local found = false

				local _, _, rank = spellSubName:find("(%d+)")
				rank = tonumber(rank)
				...
				if rank and rank > 0 then
					for index = 1, #CurrentSpells.Name, 1 do
						if (CurrentSpells.Name[index] == spellName) then
							...
							if CurrentRank < rank then
								...
								CurrentSpells.subName[index] = spellSubName
							end
							break
						end
					end
					...
					--
					-- The highest ranks of each of the numbered rank spells are inserted into the table
					if (not found) then
						table.insert(CurrentSpells.ID, spellID)
						table.insert(CurrentSpells.Name, spellName)
						table.insert(CurrentSpells.subName, spellSubName)
					end
				end
			end
		end
	...
	--
	-- We update the list of spells with the new ranks
	for spell=1, #Necrosis.Spell, 1 do
		for index = 1, #CurrentSpells.Name, 1 do
			...
			if spellName then
				for index = 1, #Necrosis.Spell, 1 do
				  if Necrosis.Spell[index].Name == spellName then
						self:MoneyToggle()
						NecrosisTooltip:SetSpellBookItem(j, BOOKTYPE_SPELL)
 ...
 --    local spellName = GetSpellInfo(spellID)
	...
	-- 			if Necrosis.Spell[index].Name == spellName then
	-- 				self:MoneyToggle()
	-- 				NecrosisTooltip:SetSpellBookItem(spellID, 1)
	-- 				local _, _, ManaCost = NecrosisTooltipTextLeft2:GetText():find("(%d+)")
	-- 				if not Necrosis.Spell[index].ID then
	-- 					Necrosis.Spell[index].ID = spellID
	-- 				end
	...
	--
	-- We update the duration of each spell according to its rank
  ...
  --
	-- WoW 2.0: Buttons must be secure to be used.
	-- Each use goes through the definition of attributes to the button, the UI being responsible for managing the click event.
...
function Necrosis:CreateMenu()
	...
	-- We hide all the icons of the spells
	...
	-- We hide all the curses icons
	print("HERE I AM")
	-- table.foreach(NecrosisConfig, print)
