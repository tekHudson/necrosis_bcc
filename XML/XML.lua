...
------------------------------------------------------------------------------------------------------
	...
	if not frame then
		frame = CreateFrame("Button", "NecrosisSpellTimerButton", UIParent)
	end
...
------------------------------------------------------------------------------------------------------
	...
	if not frame then
		frame = CreateFrame("Button", "NecrosisButton", UIParent)
	...
	frame:SetScript("OnUpdate", function(self, elapsed) Necrosis:OnUpdate(elapsed) end)
...
local function CreateStoneButton(stone)
	...
	local frame = CreateFrame("Button", "Necrosis"..stone.."Button", UIParent)
	...
	-- Soul Stone Specific Attributes
	-- They allow you to cast the stone on yourself if there is no target and out of combat
...
local function CreateMenuButton(button)
	...
	local frame = CreateFrame("Button", "Necrosis"..button.."Button", UIParent)
	...
	-- Header du bouton
	local b = CreateFrame("Frame", "Necrosis"..button.."0", UIParent)
	b:ClearAllPoints()
...
function Necrosis:CreateMenuBuff(i)
	...
	if not frame then
		frame = CreateFrame("Button", "NecrosisBuffMenu"..i, UIParent)
...
function Necrosis:CreateMenuPet(i)
	...
	if not frame then
		frame = CreateFrame("Button", "NecrosisPetMenu"..i, UIParent)
...
function Necrosis:CreateMenuCurse(i)
	...
	if not frame then
		frame = CreateFrame("Button", "NecrosisCurseMenu"..i, UIParent)
